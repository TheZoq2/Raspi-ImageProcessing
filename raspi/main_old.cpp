...
Coord getCoordsFromPixel(int pixel, int rows)
{
    int result[2];
    result[0] = (pixel % rows) + 1;
    result[1] = pixel - floor(pixel / rows);

    Coord coord(result[0], result[1]);
    return coord;   
}
...
int main ( int argc,char **argv ) 
{
    ...
    for(int i = 0; i < frameWidth * frameHeight; i++)
    {
        ...
        if(inBounds == true)
        {
            ...
            Coord coord = getCoordsFromPixel(i, frameHeight);
            xSum += coord.x;
            ySum += coord.y;
        }
    }
    ...
    int xCenter = xSum / inBoundAmount;
    int yCenter = ySum / inBoundAmount;
    ...
    std::cout << "Object is in: {" << xCenter << ", " << yCenter << "}" << std::endl;
